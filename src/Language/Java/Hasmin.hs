module Language.Java.Hasmin where
...
renderCode :: JavaCode -> String
renderCode (CodeLines ls) = unlines ls
renderCode (CodeString s) = s
renderCode (CodeSyms is) = unlines $ map printC is

renderMethod :: Method -> String
renderMethod m = unlines $ [
 unwords [".method", mattrs m, renderMethodSig m],
...
renderMethodSig :: Method -> String
renderMethodSig (Method attrs name ret args d l c) = renderSig name ret args
